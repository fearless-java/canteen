...
if (likedReview && (likedReview as any).student.id !== session.user.id) {
  ...
  const targetName = reviewData.dish?.name || reviewData.stall?.name || '你的评价';
...
export default function MessagesPage() {
  ...
  const getActionText = (message: MessageData) => {
    switch (message.type) {
      case 'review_reply':
        return <><span className="font-semibold text-black">{message.actorName || '商家'}</span> 回复了你的评价</>;
      case 'review_like':
        return <><span className="font-semibold text-black">{message.actorName || '同学'}</span> 点赞了</>;
      case 'system':
